...
credits:
  - name: "sgruendel"
    ...
    for: "First reported the GPU symlink never being rewritten and opened PR #7129"
  - name: "RonildoBraga"
    url: "https://github.com/RonildoBraga"
    for: "Showed on 4.0.2 that the missing sudo on voxtype setup gpu --enable leaves the CPU backend in place"
  - name: "kurenn"
...
faq:
  ...
  - q: "Can I use dictation on an older laptop without AVX2?"
    a: "Not with the packaged voxtype-bin. Both 0.7.5 and 1.0.1 ship only AVX2 and AVX-512 builds, so the binary dies with SIGILL on pre-Haswell CPUs. Voxtype's maintainer said in September 2026 that pre-AVX2 builds were in testing, without a ship date."
  - q: "Dictation works but my music keeps playing. Is that a bug?"
    a: "Yes, on voxtype-bin 0.7.5. The shipped config sets pause_media = true, but that path shelled out to playerctl, which Omarchy 4 does not install. According to Voxtype's maintainer, 1.0.0 replaced it with a direct D-Bus call, so upgrading the package fixes it."
...
If that prints nothing, stop. The packaged `voxtype-bin` ships only AVX2 and AVX-512 builds (true of both 0.7.5 and 1.0.1), and `/usr/bin/voxtype` points at the AVX2 one, so it aborts with `FATAL: Illegal CPU instruction (SIGILL)` on every run. The installer does not check, so you end up with a package that can never work. This is reported in issue #7883 and issue #8312, both still open on 4.0.4. PR #7890 would add a pre-flight refusal but has not merged.
...
The installer calls `voxtype setup gpu --enable` as your user, and an unprivileged process cannot rewrite the root-owned symlink. A trailing `|| true` hides the error, so the install still reports success and still shows the "Voxtype Dictation Ready" notification. Reported in issue #7099 and issue #11110; PR #7129 is open.
...
Omarchy enables Vulkan whenever any ICD file exists, which is not the same as the device being usable. On Haswell-era Intel graphics using the `hasvk` driver, ggml refuses the device at model load, the daemon aborts, and systemd restarts it every five seconds. Issue #11370 reports roughly 127,000 core dumps from this loop.
...
Log out and back in afterwards. Migration `1787865477.sh` in 4.0.2 removes the blanket `input` group grant, keeping it only for xpadneo or ydotool users. Voxtype's own hotkey listener opens `/dev/input/event*` itself rather than going through Hyprland, so once the group is gone it logs `No keyboard device found in /dev/input/` while otherwise looking healthy. Found by kurenn in issue #12013. The stock Omarchy setup is unaffected, because the shipped config has `[hotkey] enabled = false` and the compositor bindings do not need the group.
...
Open a text editor, hold `F9`, say a sentence, release. The text should land at the cursor. The mic glyph in the bar lights up as active while recording, then changes to an hourglass while transcribing.
...
**Keys fire while text is being typed.** Releasing `Super` last after `Super + Ctrl + X` makes the transcription arrive as a burst of modifier combinations. Issue #4159 collects workarounds using a Hyprland submap that swallows modifiers during output; Voxtype's maintainer said there that pre- and post-output hooks were his planned upstream answer. The simplest avoidance is to dictate with bare `F9`.

**F9 collides with your editor.** It is bound by default once Voxtype is installed. Add `hl.unbind("F9")` to your Lua config on 4.x, or `unbind = , F9` on 3.x, as r3quie noted in issue #5882.

**Media does not pause.** On `voxtype-bin` 0.7.5 the `pause_media` default shells out to `playerctl`, which Quattro removed and the installer never adds. Issues #7135 and #9243 both record `playerctl not found or failed to run` in the journal. Voxtype's maintainer says 1.0.0 switched to a D-Bus call and recommends upgrading the package over reinstalling `playerctl`.

**CJK text arrives corrupted.** Switch `[output] mode` away from `type` in `~/.config/voxtype/config.toml`; the reporter used `paste`. Issue #10050 traces this to how `wtype` assigns characters to temporary keycodes, which some clients read as physical keys.

**Pre-AVX2 hardware.** There is no packaged path on 4.0.4. Voxtype's maintainer said on issue #8312 that pre-AVX2 binaries would start with 1.0.1, but the 1.0.1 release notes list only AVX2 and AVX-512 builds, and his later comment on issue #7883 says they are still in testing.
