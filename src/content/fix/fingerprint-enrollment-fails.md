...
answer: "Run `fprintd-list \"$USER\"` first. If it prints a device, you are on 4.0.0 to 4.0.2 or holding a stale claim: update to 4.0.3 or later, run `sudo systemctl stop fprintd`, and re-run `omarchy setup security fingerprint`. If it prints No devices available, libfprint has no driver for your reader and retrying can never work."
appliesTo:
  from: "3.x"
status: workaround
...
faq:
  ...
  - q: "Did my fingerprint setup break when I upgraded to Omarchy 4?"
    a: "It can have. 4.0.0 migrated machines from libfprint-git back to stock libfprint, which dropped readers that only the upstream snapshot covered. 4.0.3 reversed that, but you have to re-run the setup to get libfprint-git back."
  - q: "Is it safe to leave a failed run half-configured?"
    a: "On 4.x yes. The setup writes the PAM stacks only after enroll and verify both pass, so a failed enrollment touches nothing in /etc/pam.d. On 3.x the PAM edits came first, so check those files."
...
If `fprintd-list` prints a device path, the hardware is drivable and your failure is fixable. Carry on with steps 2 to 4. If it prints `No devices available`, stop here and go to step 5. That is the same enumeration gap behind `Impossible to enroll: GDBus.Error:net.reactivated.Fprint.Error.NoSuchDevice: No devices available`.
...
Releases 4.0.0, 4.0.1 and 4.0.2 install stock `libfprint` and remove `libfprint-git` if it is present. 4.0.3 reversed that through [PR #10442](https://github.com/omacom/omarchy/pull/10442) by powderluv: setup now installs `libfprint-git` for every reader in a single `--ask 4` pacman transaction, which is the snapshot that carries readers Arch's release has not caught up with yet. Updating alone does not move you, because the repair migration only installs `libfprint-git` on a machine left with no libfprint at all. Re-running the setup is what swaps the package.

**3. Clear a stale device claim before you retry.**

An interrupted enroll or verify leaves fprintd holding the device, and everything afterwards fails instantly with `AlreadyInUse`. `fprintd.service` is D-Bus activated, so stopping it is enough: the next attempt starts a clean daemon, and enrolled prints survive.
...
The wizard tells you to keep moving your finger, which is advice for swipe sensors. Most current readers are press sensors: put the finger down flat and hold it for about a second per stage. Goodix 53xc firmware also disables itself temporarily after repeated touches, logging `Device disabled to prevent overheating`, and fast retry loops extend the lockout. Wait a minute or reboot rather than hammering it.
...
| Synaptics/Validity `06cb:009a` | ThinkPad T480, T480s, T490, X1 Carbon 6th | listed under known unsupported devices in libfprint's hwdb | `open-fprintd` plus `python-validity` plus `fprintd-clients-git` |
| ELAN `04f3:0c4b` | ThinkPad E14 and E15 Gen 3 and Gen 4 | enrolls, then scores zero on every verify | `libfprint-tod` plus Lenovo's ELAN plugin |
...
Every one of those routes is unofficial, pulls AUR packages, and in two cases a vendor binary. Treat them as your own maintenance burden. Once you are on a replacement stack, do not re-run the stock setup: it tries to install `fprintd`, which conflicts with `fprintd-clients-git`, and the whole command aborts. [PR #8757](https://github.com/omacom/omarchy/pull/8757) would make setup detect an existing fprintd-compatible backend and leave it alone, but it was still open at 4.0.4.
...
v3.8.4 asked the second question instead: its `check_fingerprint_hardware` ran `fprintd-list` and bailed out with `No fingerprint sensor detected.` when the list was empty. 4.0.0 replaced that with the sysfs probe, which is what lets the menu hide the entry on machines with no reader, but it also means the enroll step is now the first moment anything consults fprintd. So an unsupported reader gets all the way to `fprintd-enroll` and comes back with retry advice that cannot work. The setup script's own comments acknowledge this class of failure for Elan sensors outside the driver table. [PR #11842](https://github.com/omacom/omarchy/pull/11842) and [PR #11047](https://github.com/omacom/omarchy/pull/11047) both propose an `fprintd-list` gate before installing packages; neither had merged as of 4.0.4.

The libfprint package choice has also moved twice. 3.x installed `libfprint-git`. A migration shipped with 4.0.0 swapped everyone back to stock `libfprint`, on the grounds that release 1.94.100 had absorbed the driver the snapshot was carrying. 4.0.3 went back to `libfprint-git` and rewrote that migration to only repair machines the two-step swap had left with fprintd and no library. If your reader worked on 3.x and stopped after the Quattro upgrade, that swap is the first thing to suspect.

One thing 4.x does better: PAM configuration comes last, only after enroll and verify both succeed. On 3.x the PAM edits ran before enrollment, so a failed setup could leave `pam_fprintd.so` in your sudo and polkit stacks with no print to match, which makes `sudo` sit and wait on a reader that will never answer.
