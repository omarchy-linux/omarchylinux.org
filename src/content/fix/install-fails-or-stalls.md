...
sources:
  ...
  - url: "https://github.com/omacom/omarchy/issues/7867"
    title: "Issue #7867: Dual-boot install creates a redundant ESP instead of reusing the existing Windows one"
    kind: issue
    author: "ThePeteJames"
    date: "2026-08-23"
  - url: "https://github.com/omacom/omarchy/issues/9450"
    title: "Issue #9450: Omarchy install ends with cryptic VFS: Cannot open root device or grub_memalign:552:out of memory"
    kind: issue
    author: "jsuchal"
    date: "2026-08-31"
...
credits:
  ...
  - name: "HalbonLabs"
    ...
    for: "Showed that pre-creating an ESP on the target disk fixes free-space installs"
  - name: "jsuchal"
...
faq:
  ...
  - q: "Can I install an older ISO and upgrade instead?"
    a: "Yes. Several people in issue #6985 installed a 3.x ISO and ran the Quattro upgrade afterwards to get past an ISO-side bug."
...
1. Get a shell on the live ISO. The installer runs on tty1, so press `Ctrl + C` at the failure screen or switch to another tty with `Ctrl + Alt + F2`.
...
4. **Free space install dies at the EFI partition.** The signature is `mounting the ESP failed (exit 32)` preceded by `Can't find a SQUASHFS superblock on nvme0n1p3`. The reliable workaround, reported on issue #7515 against 4.0.3, is to give the target disk an EFI partition before you start. From Windows, `diskpart` on that disk, then `create partition efi size=1024` and `format quick fs=fat32`. Re-run the installer and pick the free space as before. Damian626's alternative on issue #7263 patches the installer's configurator script in the live session, adding `udevadm settle` and `sleep 2` after the ESP is created and forcing `mount -t vfat`, then `umount -R /mnt 2>/dev/null` and `./.automated_script.sh` to start over. Several people confirmed it worked.
...
6. **The ISO never gets as far as the installer.** `Kernel panic - not syncing: VFS: Unable to mount root fs on unknown-block(0,0)` or `grub_memalign:552:out of memory` mean the firmware is blocking the boot. Secure Boot and TPM must be off, as the manual states in [Getting Started](https://omarchy.org/manual/getting-started/). On a Lenovo Yoga 7 ProX the setting was named `Intel Platform Trust Technology (Intel PTT)`, not TPM, which is why issue #9450 was filed. See [secure boot violation or will not boot UEFI](/fix/secure-boot-violation-or-wont-boot-uefi/).
...
The free space path creates its own new ESP rather than reusing an existing one, which issue #7867 documents with a full partition table. On disks with no ESP at all the mount step fails outright.
...
A blunt option that several people in issue #6985 used successfully: install a 3.x ISO, then run the [3 to 4 Quattro upgrade](/upgrade/3-to-4-quattro/). ISO-side bugs do not apply if you never boot that ISO.
