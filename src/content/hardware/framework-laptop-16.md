...
answer: "Silver. The Framework Laptop 16 installs and runs on Omarchy 4.x, and the distro ships model-specific enablement: a DMI detector, the qmk-hid package, a keyboard udev rule and theme-synced backlighting. Two open bugs matter: omarchy theme set can hang when a VIA keyboard is attached, and opening Activity wakes a suspended discrete graphics module."
appliesTo:
  from: "3.x"
  to: "4.0.4"
status: partial
...
Silver. The machine installs and runs, nothing in the tracker points at a dead subsystem, and the distro ships model-specific enablement for it. Two things keep it off gold. First, the evidence is thin: six issues and four discussions across the whole tracker name this laptop, so most subsystems here are marked unknown rather than working. Second, two live bugs are open on 4.0.4, and both are Omarchy bugs rather than hardware faults.

Checked against the v4.0.4 and v3.8.4 source snapshots on 2026-09-16.

## What works

The Ryzen AI 300 configuration drives its displays from the integrated Radeon 890M through `amdgpu`, reported directly by the owner in [#11184](https://github.com/omacom/omarchy/issues/11184) on Omarchy 4.0.3 with kernel 7.2.3. Wi-Fi associates on a fresh 4.x install; the only Wi-Fi report, [#10406](https://github.com/omacom/omarchy/issues/10406), is about the panel refusing to forget a mistyped network, not about the radio.
...
Subsystems with no Omarchy-specific report either way: fingerprint, webcam, audio, Bluetooth, touchpad, hibernate and system suspend. Absence of bug reports is weak evidence, so this page does not claim them.

## What breaks

**Install fails with `error: target not found: qmk-hid`.** Reported by @bsl in [#6423](https://github.com/omacom/omarchy/issues/6423) against the 3.8.4 ISO. The Framework 16 hook asked pacman for `qmk-hid`, but the package was never listed in the ISO builder's sources, so it was absent from the offline mirror. Commit [edce5809](https://github.com/omacom/omarchy/commit/edce5809df36003c96822b456f327fa79ec1cfd7) by @ryanrhughes added it to `install/omarchy-other.packages`. That file has no `qmk-hid` entry in the v3.8.4 tree and does have one from v4.0.0 onward, so 4.x installs are clear. The issue is still open on the tracker, but the cause is gone.

**`omarchy theme set` and `omarchy update` hang forever.** [#8243](https://github.com/omacom/omarchy/issues/8243), open, filed by @ykzird against 4.0.1. `omarchy-theme-set-keyboard-f16` guards only on `qmk_hid` being installed and a theme shipping a `keyboard.rgb` file, then fires five unbounded `qmk_hid` calls. Nothing checks for a Framework 16 or for a responsive VIA device. Attach another QMK/VIA keyboard, a Keychron K8 Pro in that report, and `qmk_hid` blocks indefinitely. Because a 4.0.1 migration re-applies the theme, the whole update wedges there and looks bricked. Unplug the offending keyboard and it completes at once. The reporter suggests wrapping the calls in `timeout`; that has not shipped as of 4.0.4.

**Opening Activity wakes the discrete graphics module.** [#11184](https://github.com/omacom/omarchy/issues/11184), open, on a Laptop 16 with a Ryzen AI 9 HX 370 and an RTX 5070 module. Omarchy's `config/btop/btop.conf` sets `shown_gpus = "nvidia amd intel"`, and btop initialises NVML at startup whenever that list contains `nvidia`, even with no GPU box displayed. The card leaves runtime suspend about two seconds after Activity opens and stays awake until roughly fifteen seconds after you close it. Setting `shown_gpus = "amd"` in your own btop config kept the card suspended in the reporter's measurements.

**keyboard.frame.work cannot reach the input modules.** Discussion [#4073](https://github.com/omacom/omarchy/discussions/4073) by @liancheng. Omarchy's shipped rule, `default/udev/framework16-qmk-hid.rules`, is a single line matching `hidraw` devices with vendor `32ac` and product `0012`, the ANSI keyboard module. The numpad module is `32ac:0014` and is not covered, and the WebHID configurator wants rules on the USB device rather than hidraw. The discussion also notes that QMK's own `50-qmk.rules` fails `udevadm verify` on Arch because it references a `plugdev` group that does not exist.
...
If you fit a discrete module, `install/hardware/nvidia.sh` installs `nvidia-open-dkms` for an RTX 5070, sets `nvidia_drm modeset=1` and adds the NVIDIA modules to the initramfs. `omarchy-hw-hybrid-gpu` then counts GPUs and unlocks the hybrid GPU toggle. None of that is Framework-specific; see [hybrid GPU laptops](/hardware/hybrid-gpu/).

**3.x versus 4.x.** The scripts are the same, the paths moved. On v3.8.4 they lived at `install/packaging/framework16.sh` and `install/config/hardware/framework/qmk-hid.sh`. On 4.x both sit under `install/hardware/`. The detector and the udev rule are byte-identical between the two trees.
