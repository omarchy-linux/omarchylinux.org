...
credits:
  ...
  - name: "Elshayib"
    ...
    for: "Found that PCI by-path names in AQ_DRM_DEVICES are split on every colon and kill the session"
...
faq:
  ...
  - q: "Should I set AQ_DRM_DEVICES?"
    a: "Only as a last resort. Never use a /dev/dri/by-path name: Aquamarine splits the value on every colon, so a PCI path becomes garbage and the session login-loops with no error. If you set it at all, list both cards by their card node."
...
What works on a stock install: both drivers get installed, the session starts, external monitors on either card light up, Vulkan works on both, and `prime-run` offloads applications to the discrete card. Games and CUDA are the happy path.

What is unreliable: video decode routing in browsers, suspend and resume, and discrete GPU runtime power management. Several reports show the discrete card logging zero milliseconds of runtime suspension across a whole session.

The kernel side barely differs between 3.x and 4.x, but 4.0.0 changed the session environment. Before Quattro these NVIDIA variables often were not set at all, because `os.execute` inside the Hyprland Lua config did not work as intended. The 4.0.1 fix to that helper switched them on for the first time on many machines, which is why users date their video problems to 4.0.1.
...
`default/hypr/nvidia.lua` sets the per-session environment. The GSP branch exports `NVD_BACKEND=direct`, `LIBVA_DRIVER_NAME=nvidia` and `__GLX_VENDOR_LIBRARY_NAME=nvidia`; the legacy branch exports `NVD_BACKEND=egl` and `__GLX_VENDOR_LIBRARY_NAME=nvidia` only. The detectors `omarchy-hw-nvidia`, `omarchy-hw-nvidia-gsp` and `omarchy-hw-nvidia-without-gsp` read cached IDs under `/sys/bus/pci/devices` instead of running `lspci`, because reading PCI config space wakes a runtime-suspended card out of D3cold and that wake alone outlasts Hyprland's 1.5 second config reload budget. That landed in 4.0.0.
...
| [#3242](https://github.com/omacom/omarchy/issues/3242) random full freeze from RTD3 PCI reallocation loop | Optimus laptops, GTX 1050/1060 class | workaround | not fixed |
| [#5274](https://github.com/omacom/omarchy/issues/5274) NVRM assertion storm and crash on lid close under s2idle | Hybrids in supergfxctl Hybrid mode | open | not fixed |
| [#4891](https://github.com/omacom/omarchy/issues/4891) compositor freeze on resume, stale frame, dead input | Intel plus NVIDIA, nvidia-open-dkms | open | not fixed |
...
| [#12187](https://github.com/omacom/omarchy/issues/12187) hard freeze about five seconds after login on 4.0.4 | NVIDIA laptops on prebuilt `nvidia-open`, not DKMS | open | not fixed |
...
The first three rows are one bug seen from three angles. Where the integrated card drives every display, `nvidia.lua` still exports `LIBVA_DRIVER_NAME=nvidia`, so libva loads the NVDEC driver on a card that is not compositing. Reporters measured corrupted frames, wallpaper punch-through, or `vaEndPicture failed, VA error: internal decoding error` in a loop that never completes a frame. Switching that one variable to `iHD` fixed it on every machine in the thread. The NVDEC backend also advertises zero encode entrypoints, so hardware encode is silently lost at the same time.

A related trap: `--disable-features=VaapiVideoDecoder` does not fix it. One reporter confirmed the flag live in the process with no change, then spent a session chasing unrelated theories. The variable is still exported and the NVIDIA VA driver still loads.
...
**4. For suspend problems,** enable `nvidia-suspend.service`, `nvidia-resume.service` and `nvidia-hibernate.service`. Omarchy installs the driver but leaves those units disabled. If lid close still crashes, try `mem_sleep_default=deep` on the kernel command line. See [suspend will not resume](/fix/suspend-wont-resume-s2idle/).
