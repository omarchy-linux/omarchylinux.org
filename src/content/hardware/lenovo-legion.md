...
description: "Lenovo Legion 5, 7 and Pro on Omarchy 4.0.4: bronze. NVIDIA hybrid graphics mostly work, suspend and brightness are the recurring failures."
answer: "Bronze. A Legion installs and runs, but it is a hybrid NVIDIA laptop with no Legion specific enablement in Omarchy 4.0.4. Expect to fight suspend: several owners report no sleep on lid close and hard freezes after resume. Brightness keys are dead on some 2026 models, and one Legion Pro 7 has silent internal speakers. Buy older Ampere or Ada units over brand new ones."
appliesTo:
  from: "4.0.0"
  to: "4.0.4"
status: partial
...
quirkScripts:
  ...
  - name: "default/hypr/nvidia.lua"
    ...
    note: "Sets NVD_BACKEND, LIBVA_DRIVER_NAME=nvidia and __GLX_VENDOR_LIBRARY_NAME=nvidia whenever an NVIDIA GPU exists, without asking which GPU renders. This is the cause of the hybrid video corruption below."
  - name: "bin/omarchy-toggle-hybrid-gpu"
    url: "https://github.com/omacom/omarchy/blob/v4.0.4/bin/omarchy-toggle-hybrid-gpu"
    note: "Installs supergfxctl on demand and flips between Hybrid and Integrated. Not Asus only: it works on any machine supergfxd can drive."
  - name: "bin/omarchy-hw-display"
    url: "https://github.com/omacom/omarchy/blob/v4.0.4/bin/omarchy-hw-display"
    note: "Picks the backlight device. Its candidate list is gmux, amdgpu_bl*, intel_backlight and acpi_video*. It has no case for nvidia_wmi_ec_backlight, which matters on newer Legions."
  - name: "bin/omarchy-hw-match"
    url: "https://github.com/omacom/omarchy/blob/v4.0.4/bin/omarchy-hw-match"
    note: "The DMI matcher used by every model quirk. No caller in the v4.0.4 tree passes a Legion or Lenovo pattern."
...
faq:
  - q: "Is a Lenovo Legion a good machine for Omarchy?"
    a: "It is workable, not carefree. Nothing in Omarchy 4.0.4 targets a Legion, so you get the generic NVIDIA path and nothing else. The recurring complaints are suspend and resume, brightness control on 2026 models, and hybrid graphics environment variables that hurt video playback."
  ...
  - q: "Which Legion generation is safest?"
    a: "An Ampere or Ada unit, roughly RTX 30 and RTX 40 series. The worst reports on 4.0.x come from Arrow Lake HX plus RTX 50 Max-Q machines, where resume from deep sleep freezes the desktop. Very old Pascal Legions such as the Y530 fall back to the nvidia-580xx legacy driver."
...
Bronze. A Legion installs and runs Omarchy, and the GPU side is no worse than any other NVIDIA Optimus laptop, but 18 of the 22 tracker issues that name a Legion are still open and the cluster is dominated by suspend and display problems. Omarchy 4.0.4 ships no Legion specific enablement at all. There is no Legion entry in `install/hardware`, and no caller of `bin/omarchy-hw-match` passes a Lenovo or Legion pattern. What you get is the generic NVIDIA install script plus whatever your iGPU vendor gets.
...
The hybrid GPU toggle is available to you. `omarchy-toggle-hybrid-gpu` installs `supergfxctl` on first use, writes `/etc/supergfxd.conf` and flips between Hybrid and Integrated with a reboot. It is often described as an Asus feature, but the detector in `omarchy-hw-hybrid-gpu` simply counts display class PCI devices, so a Legion qualifies. See [/hardware/hybrid-gpu/](/hardware/hybrid-gpu/) for what that costs you.
...
**Suspend, repeatedly.** This is the headline problem. Issue #2501, open since October 2025 and still open, is a Legion 5 15ARH05H that never suspends on lid close even after uncommenting the `HandleLidSwitch` settings in `logind.conf`. Other owners piled on, including a second Legion 5 owner who said no Linux distribution would suspend on that machine. One reporter got it working by adding `mem_sleep_default=deep`, another by leaving the performance power profile. On newer hardware it fails in the other direction: issue #9765 is a Legion Pro 7 with a Core Ultra 9 275HX and an RTX 5080 Max-Q that resumes from ACPI S3 into a frozen session, with `spd5118_resume` returning an error and the shell reporting no outputs. Issue #10626 is a Legion Y7000P IRX9 with an RTX 4060 where the journal shows Hyprland blocked inside NVIDIA modesetting for over four minutes after the kernel finished resuming. Both run driver 610.57.04. Start at [/fix/suspend-wont-resume-s2idle/](/fix/suspend-wont-resume-s2idle/) and the [System sleep](https://omarchy.org/manual/system-sleep/) manual chapter.

**Brightness keys on 2026 models.** Issue #11665 traces dead brightness keys on a Legion 5 15IAX11 all the way down. The key emits the right event, Hyprland dispatches it, `brightnessctl` writes and reads back both `intel_backlight` and `nvidia_0`, and the panel ignores all of it. The cause is an EC firmware bug behind `nvidia_wmi_ec_backlight`, reported upstream as kernel bugzilla 221430. No compositor level fix exists. The reporter also notes that `omarchy-hw-display` never considers `nvidia_0` at all, which we confirmed in the v4.0.4 script.

**Hybrid video decode.** `default/hypr/nvidia.lua` exports `LIBVA_DRIVER_NAME=nvidia` and `__GLX_VENDOR_LIBRARY_NAME=nvidia` for any machine with an NVIDIA GPU, and `autostart.lua` pushes that environment into the whole session. On a Legion where the Intel or AMD iGPU drives the panel, every VA-API client then decodes on the wrong device. Two Legion owners filed it: issue #8989 from a Legion 7i Pro and issue #9890 from a Legion 5 15IAX10, which reports corrupted video on live streams and one Chromium GPU process abort. The workaround both point at is overriding the variable back to `iHD` or `mesa` in `~/.config/hypr/envs.lua`. See [/hardware/nvidia/](/hardware/nvidia/).

**Audio on at least one model.** Issue #12086, filed on 4.0.4 with the bundled `linux-omarchy` kernel, has silent internal speakers on a Legion Pro 7 16IRX8H. The TAS2781 amplifier loads and binds its I2C device, but the ALC287 codec does not bind the amplifier component on a normal boot. Reloading `snd_hda_intel` with a codec SSID override makes it bind. The reporter is explicit that this is a diagnostic override, not a confirmed fix, and that it is unproven whether the defect is Omarchy's or upstream's. One machine, one week old report. More at [/fix/no-sound-from-laptop-speakers/](/fix/no-sound-from-laptop-speakers/).
...
DMI values people reported in these issues are the Lenovo type codes `82JW` (Legion 5 15ACH6), `82WQ` (Legion Pro 7 16IRX8H) and `83DG` (Legion Y7000P IRX9), with the marketing names appearing in the DMI family or version fields. `omarchy-hw-match` checks `product_name` and `product_family`, so a future Legion quirk would most likely match on the family string.

## Variants

Prefer an RTX 30 or RTX 40 series Legion with an AMD or Intel HX chip from roughly 2021 to 2024. Those generations appear in the tracker with ordinary hybrid graphics complaints.

Be careful with the newest Arrow Lake HX plus RTX 50 Max-Q machines, currently the Legion Pro 7 and Legion 5 15IAX10 and 15IAX11 class. Every hard freeze after resume and the dead brightness EC report comes from that group.

Very old Legions such as the Y530 with Pascal graphics drop below the GSP cutoff and get `nvidia-580xx-dkms` instead of the open driver. That works but is a legacy branch.
...
- Check your NVIDIA generation. RTX 50 Max-Q plus Arrow Lake HX is the risky combination on 4.0.x.
...
- If video looks corrupted in Chromium or Electron apps, override `LIBVA_DRIVER_NAME` in `~/.config/hypr/envs.lua` before assuming your GPU is broken.
