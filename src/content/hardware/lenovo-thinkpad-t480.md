...
answer: "Silver. A T480 or T480s runs Omarchy 4.0.4 well: i915 graphics, Intel 8265 Wi-Fi, HDA audio and both cameras come up with no extra work. Two things bite. The bar and power panel report only BAT0, so the second pack is invisible, and the Synaptics Validity 06cb:009a fingerprint reader has no libfprint driver, so Setup > Security > Fingerprint always fails."
appliesTo:
  from: "3.x"
  to: "4.0.4"
status: partial
...
dmi: ["20L8S27E04", "20L7S2QV00", "20MF000DUS", "ThinkPad T480s"]
year: "2018"
...
credits:
  ...
  - name: "vcelletti"
    ...
    for: "Showed that the bar icon and the power panel read different battery devices on a T480"
  - name: "pony-montana"
...
- Both cameras, the regular Chicony one and the IR camera on IR-equipped units, bind to `uvcvideo`.
...
3.x-era installer failures on this model exist and appear fixed by time rather than by a named release: a TPM initialization hang after the LUKS prompt in [#4296](https://github.com/omacom/omarchy/issues/4296), a Limine install crash that left a machine unbootable in [#5330](https://github.com/omacom/omarchy/issues/5330), and an SDDM stall after the 3.3.0 update in #4120. If you install 4.0.4 today, none of them is a reason to hesitate.
...
On DMI, ThinkPads put the machine type in `product_name` and the friendly name in `product_version` and `product_family`. Confirmed T480s strings from fetched issues are `20L8S27E04`, `20L7S2QV00` and `20MF000DUS`; the family string reads `ThinkPad T480s`. If you write your own quirk, match on the family, not the machine type, because every configuration has a different type code.
...
Libreboot or Coreboot units without an EFI payload will not install: [#4208](https://github.com/omacom/omarchy/issues/4208) is a T480 in exactly that state, and the installer assumes UEFI.
