...
quirkScripts:
  ...
  - name: "vulkan.sh"
    ...
    note: "Installs vulkan-intel whenever lspci reports an Intel display device, with no generation check. On Haswell that lands a hasvk ICD, which is enough for omarchy-hw-vulkan to report Vulkan as available. See issue #11370."
...
sources:
  ...
  - url: "https://omarchy.org/manual/terminal/"
    ...
    author: "omacom"
    date: "2026-09-16"
  - url: "https://github.com/peteonrails/voxtype/issues/740"
    title: "voxtype issue #740: 1.1.0-rc4 x86_64 baseline binary still contains BMI2 and SIGILLs on pre-Haswell CPUs"
    kind: issue
    author: "ironbract"
    date: "2026-09-08"
...
This page covers the X220 and X230, the T420 and T430, and by extension the Haswell T440 series. Several of the cited reports come from machines just outside that range: the Broadwell X250 and T450, the Skylake T460s, and two Intel MacBooks of the same GPU generations. They all ride the same generic Intel and power paths. Everything was checked against the v4.0.4 source tree and the issue tracker on 2026-09-16.
...
**Dictation on pre-AVX2 chips.** Issue #8197 reports the prebuilt voxtype binary dying with SIGILL on an Ivy Bridge i5-3210M, with no warning from the installer. The upstream maintainer said in the thread that pre-AVX2 binaries would ship from voxtype 1.0.1. That did not happen: 1.0.1 shipped on 2026-08-31 with AVX2 and AVX-512 x86_64 builds only. A `baseline` x86_64 binary does appear in the 1.1.0 release candidates, but voxtype issue #740 is open and reports that the rc4 baseline build still carried BMI2 and still faulted on pre-Haswell hardware; rc5 was tagged as an attempted fix and has no bare-metal confirmation yet. The workaround that the #8197 reporter says works today is building from source with `RUSTFLAGS="-C target-cpu=native"`, at roughly nine seconds per utterance. On Haswell there is a second trap: issue #11370, filed from a Haswell MacBook Pro, describes `omarchy-hw-vulkan` seeing any Vulkan ICD as good enough, enabling GPU offload on a Mesa hasvk device that cannot support it, and leaving a systemd restart loop that produced roughly 127,000 core dumps. The same install path runs on a Haswell ThinkPad. See [dictation not working](/fix/dictation-voxtype-not-working/).
...
**Brightness can trap at zero.** Issue #8523 is open. It was filed from a 2012 MacBook Pro, but the mechanism is the backlight driver, not the chassis: on panels using `acpi_video0` with a `max_brightness` of 15, stepping down to 0% leaves the backlight stuck, because a 1% step rounds back to raw zero. A commenter reproduced it on 4.0.2 with Ivy Bridge HD 4000. Do not hold the brightness-down key.
...
Prefer an X230 or T430 over an X220 or T420 if the price is close, mostly for the newer chipset and USB 3. Prefer any of them with 8 GB or more and an SSD. The third-party X220 writeup makes the same point bluntly: a cheap SATA or mSATA SSD does more for responsiveness than any software tweak.
...
- On a pre-AVX2 chip, expect the packaged dictation binary to die on first run. Build voxtype from source if you want it, and do not count on the speed.
