...
description: "Classic ThinkPad support on Omarchy 4.0.4: the X220 is the official potato demo, but scaling, VA-API, dictation and dual-battery reporting all need hand work."
answer: "Rate the classic ThinkPad class bronze. Omarchy boots and runs well on an X220 or X230, and the official site uses a 2 GB 2011 X220 as its old-hardware showcase. Expect real hand work: set scaling to 1 on a 1366x768 panel, install libva-intel-driver yourself, skip dictation on pre-AVX2 chips, and watch for an unresolved Mesa rendering regression on old Intel graphics."
appliesTo:
  from: "3.x"
status: partial
...
dmi: []
cpu: "Intel Core 2nd gen (Sandy Bridge) and 3rd gen (Ivy Bridge); Haswell on the T440 and T450 series"
...
quirkScripts:
  ...
  - name: "intel/video-acceleration.sh"
    ...
    note: "Matches the string \"hd graphics\" and installs intel-media-driver, which only supports Broadwell and newer. HD 3000 and HD 4000 need libva-intel-driver instead. See issue #7866."
  - name: "vulkan.sh"
...
credits:
  ...
  - name: "mikl0s"
    ...
    for: "Traced the 1366x768 scaling reset through the screensaver wake path"
  - name: "ryanrhughes"
...
faq:
  - q: "Will Omarchy run on a ThinkPad X220 with 2 GB of RAM?"
    a: "Yes, and that exact machine is the demo on the official potato page. A third-party writeup measured about 890 MB at idle on an i5-2520M X220 running Quattro. Browser and Electron work is where 2 GB runs out, so buy the 8 GB stick before you buy anything else."
  ...
  - q: "Is hardware video decode working on HD 3000 and HD 4000?"
    a: "Not out of the box on 4.0.4. The install script installs intel-media-driver, which starts at Broadwell. Install libva-intel-driver and check vainfo. Issue #7866 is open with two candidate fixes."
...
Bronze. A classic ThinkPad runs Omarchy, and the project leans on that fact: the official [old hardware page](https://omarchy.org/potato/) is a 2011 ThinkPad X220 with 2 GB of RAM. The desktop itself is not the problem. Everything around the desktop is.
...
This page covers the X220 and X230, the T420 and T430, and by extension the T440 and T450 series, checked against the v4.0.4 source tree and the issue tracker on 2026-09-16.
...
The pre-AVX2 update failure from early 2026 is fixed. Issue #4554 had owners of i5-3210M and similar chips stuck when the Limine unified kernel image build refused to run, reporting a missing list of CPU features including AVX2 and BMI2. ryanrhughes closed it on 2026-02-22 after tracing it to the `limine-mkinitcpio-hook` build and rebuilding it with compatibility flags in the Omarchy package repository. If you are installing 4.0.4 fresh you will not see it.

Everything else on the subsystem table is marked unknown on purpose. The tracker only proves what breaks, and nobody has filed a verified Wi-Fi, Bluetooth, audio, webcam or suspend report for these machines under 4.x. Absence of complaints is weak evidence, so this page does not turn it into a claim.
...
- Skip dictation until voxtype ships a pre-AVX2 build.
