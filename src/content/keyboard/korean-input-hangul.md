...
answer: "Omarchy already runs fcitx5 in every session, so you only need the engine. Run `omarchy pkg add fcitx5-hangul fcitx5-configtool`, restart the unit with `systemctl --user restart omarchy-fcitx5.service`, then open fcitx5-configtool and add Hangul to your input method group below Keyboard - English (US). Toggle with the 한/영 key or Ctrl+Space. Korean fonts are already installed."
appliesTo:
  from: "4.0.0"
  to: "4.0.4"
status: workaround
...
credits:
  - name: "MoerAI"
    ...
    for: "Worked out the D-Bus group rewrite and the xkb options for keyboards with no 한/영 key"
...
faq:
  ...
  - q: "Why is Korean missing from the installer's keyboard list?"
    a: "The installer maps each label to a console keymap, and kbd ships no Korean console keymap. Korean keyboards type Latin on the US map anyway, and Hangul comes from the input method, not the keymap. PR #9299 proposes adding Korean as a special case."
...
This page was checked against v4.0.4 (2026-09-15) on a machine installed from the 4.0.x ISO.
...
2. Restart the supervised fcitx5 so it discovers the new engine. On 4.0.x, fcitx5 runs as a user unit, so restarting it is a systemd command, not a `pkill`.
...
5. Toggle between English and Hangul with the **한/영** key on a Korean keyboard, or with **Ctrl+Space** on a keyboard that has no dedicated key. Both are fcitx5 defaults, so you do not need to configure a hotkey.
...
That call shape is the one used by the scripted installer in the community repo [yelixir-dev/omarchy-starter](https://github.com/yelixir-dev/omarchy-starter), a Korean-language first-day guide for Omarchy, and by the proposed upstream command in PR #11719. Note that it replaces the group wholesale, so if you already run another engine, add it back in the same call or use the GUI.
...
The installer is a separate gap. The keyboard picker in `install/provisioning/setup-form.sh` offers 47 layouts, including Japanese as `jp106`, but there is no Korean entry. PR #9299 by ronaldlangeveld explains why: the picker maps each label to a console keymap, kbd has no Korean console keymap, and Korean is not really a keymap problem. A Korean keyboard types Latin letters on the US map, and Hangul is produced by the input method. That PR proposes writing `KEYMAP=us` with `XKBLAYOUT=kr` and seeding a Hangul profile on first run. It was opened on 2026-08-31 and is still open.
...
**The top bar shows nothing when you switch to Hangul.** It will not. The bar's keyboard layout widget tracks Hyprland's XKB layout, and fcitx5 input methods do not change that. With a single `us` layout the widget stays hidden entirely, because it only appears when you have configured more than one layout. PR #11341 proposed showing `EN` or `KO` in that widget, but it is closed and unmerged. Watch the fcitx5 tray icon instead.

**You use Dvorak or Colemak.** Hangul input assumes a QWERTY physical mapping. Add a QWERTY entry to your fcitx5 group and switch to it before typing Korean.

**No 한/영 key and Ctrl+Space is taken.** xkb has options for this. Add `korean:ralt_hangul` and `korean:rctrl_hanja` to `kb_options` in `~/.config/hypr/input.lua` to turn Right Alt and Right Ctrl into the Hangul and Hanja keys.
