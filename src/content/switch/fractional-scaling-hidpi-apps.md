...
faq:
  ...
  - q: "Does fractional scaling make text blurry on Omarchy?"
    a: "Native Wayland apps render sharp at fractional scale. XWayland apps do not, because Omarchy sets xwayland force_zero_scaling, so they draw at 1x and the compositor upscales them. The same applies to any app you launch with --force-device-scale-factor=1."
related: [what-replaces-what, day-one-checklist]
draft: false
---

Omarchy assumes you are on a retina-class display. The shipped `~/.config/hypr/monitors.lua` sets `GDK_SCALE` to 2 and leaves the Hyprland monitor scale on `"auto"`. On a 2x panel that is right. On a 4K 27-inch monitor, a 1080p laptop, or an ultrawide, it is not, and the first thing you notice after switching from macOS or Windows is that some windows are the wrong size while others are fine.
...
**XWayland apps.** Omarchy sets `xwayland { force_zero_scaling = true }` in `default/hypr/envs.lua`, which it also did in 3.x. X11 clients therefore see the full physical resolution and draw at 1x, so they look soft when the compositor upscales them, and they take one global size across all monitors. Spotify is the app Omarchy installs that is still on XWayland by default: issue #11175 reports it being sized for one monitor and wrong on the other in a mixed-DPI setup, and proposes shipping a `spotify-flags.conf` with `--ozone-platform=wayland`. As of 4.0.4 no such file ships, and `omarchy-launch-spotify` runs `/usr/bin/spotify` with no flags. Chromium is fine here because Omarchy does ship `~/.config/chromium-flags.conf` with `--ozone-platform=wayland`.
...
```bash
GDK_SCALE=1 your-java-app
```

**Electron and Chromium at fractional scale.** Two separate reports. Issue #9904 found that the 1Password browser-unlock popup maps at a size that does not match its own layout at scale 1.6, clipping the password field, and that launching with `--force-device-scale-factor=1` makes it usable. Omarchy adopted that in 4.0.3: `bin/omarchy-launch-1password` now runs `1password --force-device-scale-factor=1`. The trade is softness, because the app renders at 1x and gets upscaled. Issue #9907 is a different defect, stale and duplicated regions in Electron secondary windows at scale 1.6, and the reporter's workaround is to disable one Chromium feature:
...
`GDK_SCALE` is still a single session-wide value, so on a mixed-DPI pair there is no correct setting for GTK apps forced through XWayland. Pick the integer that suits the monitor you work on most.
...
It takes 9 to 20 pixels and moves the Omarchy shell, GTK's text scaling factor, and your terminal font together. `omarchy display text size reset` returns to the 12px default. Running terminals in Foot keep their old size until you open a new window.
