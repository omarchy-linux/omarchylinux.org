...
answer: "Omarchy is a pre-decided Arch and Hyprland desktop: an ISO that sets up LUKS, Btrfs, Limine and Snapper, about 147 base packages, a Quickshell shell, 444 omarchy CLI commands, versioned migrations and 38 hardware quirk scripts. You give up package freshness on the stable mirror, direct pacman upgrades and per-choice control. Build Arch yourself if those matter more than the setup time."
appliesTo:
  from: "4.0.0"
  to: "4.0.4"
status: info
...
**Themes as a system.** v4.0.4 ships 22 themes under `themes/`, up from 19 in v3.8.4, and applying one retints the terminal, the shell, Chromium and the backgrounds together.

**The `omarchy` CLI.** v4.0.4 carries 444 commands in `bin/`, grouped into 67 namespaces such as `audio`, `channel`, `hw`, `snapshot` and `theme`. 74 are hidden internals. v3.8.4 had 283. Run `omarchy` with no arguments for the index, documented in the [Omarchy CLI](https://omarchy.org/manual/omarchy-cli/) chapter. Our [command reference](/reference/commands/) tracks the list per release.
...
**Package freshness on stable.** The [Updates](https://omarchy.org/manual/updates/) chapter says the stable mirror deliberately runs behind so incompatibilities get caught before they reach users. That lag is real and it is not always the documented amount. Checked on 2026-09-16, `stable-mirror.omarchy.org` had last synced on 2026-09-07, roughly nine days behind `geo.mirror.pkgbuild.com`, while the edge mirror was under an hour behind. Open issue [#9064](https://github.com/omacom/omarchy/issues/9064) reports the stable databases sitting five days stale and pacman silently accepting them. Open issue [#10732](https://github.com/omacom/omarchy/issues/10732) makes the sharper point: with the mirror frozen at its 2026-08-25 sync, stable installs stayed on Chromium 151 after Arch published 152, which carried a fix for a vulnerability Google said was being exploited in the wild. On plain Arch you would have had it the day it landed. More on this in [stable mirror lag and CVEs](/security/stable-mirror-lag-and-cves/).
...
**Direct pacman upgrades.** A pacman hook calls `omarchy-update-pacman-guard`, which aborts a direct `-Syu` and tells you to use `omarchy update`. The bypass is `sudo env OMARCHY_ALLOW_DIRECT_PACMAN=1 pacman -Syu`, and using it skips the snapshot, the migrations and the post-update hooks. Partial upgrade freedom is also coarse: discussion [#8347](https://github.com/omacom/omarchy/discussions/8347) asks for a way to update the system without pulling in every optional AUR app, and there is no built-in switch for it.

**Configuration you did not choose.** `omarchy-refresh-pacman` copies over `/etc/pacman.conf` and the mirrorlist, keeping `.bak` copies. The `omarchy refresh` family resets other config files to defaults. On a hand-built Arch, nothing rewrites your config unless you tell it to.
...
Pick **plain Arch plus Hyprland** if you need packages the day they ship, if you run security-sensitive work where browser and kernel fixes cannot wait on a mirror sync, if you already have dotfiles you like, if you want a different bootloader, filesystem or shell, or if the point of the exercise is learning how the parts fit. Also pick it if you dislike software that guards your package manager.
...
The next release is announced as Quattro RS 4.5 rather than 4.1, with DHH citing a bespoke optimized kernel build as part of the reason. A distro-built kernel would widen the gap with plain Arch further, so recheck the package-freshness section against whatever ships.
